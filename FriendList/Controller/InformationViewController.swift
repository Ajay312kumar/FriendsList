...
class InformationViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func submitBtnAction(_ sender: UIButton) {
        ...
        guard let firstNameText = firstName.text, let lastNameText = lastName.text, let contactText = contact.text else {
                navigationController?.popViewController(animated: true)
                return
            }
            
            let newFriend = Friend(firstName: firstNameText, lastName: lastNameText, contact: contactText,profilePicture:  imgView.image)
